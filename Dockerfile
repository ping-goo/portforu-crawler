...
RUN apt-get update \
 && apt-get install -y --no-install-recommends wget gnupg2 ca-certificates curl unzip \
 ...
 && CHROME_MAJOR=$(google-chrome --version | cut -d. -f1) \
 && CHROMEDRIVER_VER=$(wget -qO- "https://chromedriver.storage.googleapis.com/LATEST_RELEASE_${CHROME_MAJOR}") \
